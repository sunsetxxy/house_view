...
def get_home_url(page):
    url = 'https://esf.fang.com/house/h316-i{}/'.format(page)
    headers = {
...
def get_home_detail_infos(detail_url):
    ...
    # 解析获取相关数据
    # 所在地址
    home_location = html.xpath('//div[@class="tr-line"]//div[@class="rcont"]//text()')
...
def save_data(data):
    ...
    print(data_frame)
    data_frame.to_csv('beijing_fang.csv',header=False,index=False,mode='a',encoding='utf_8_sig')
...
def main(page):
    ...
    for url in urls:
        ...
        all_data = get_home_detail_infos(detail_url='https://esf.fang.com'+url+'l')
        data = []
